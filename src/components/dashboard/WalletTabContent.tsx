...
export default function WalletTabContent() {
  ...
  const topupPackages = useMemo(() => {
    const packages = walletSummary?.topupPackages;
    return Array.isArray(packages) && packages.length > 0 ? packages : [20000, 35000, 60000];
  }, [walletSummary?.topupPackages]);

  return (
    <main className="flex-1 overflow-y-auto px-4 md:px-6 lg:px-8 xl:px-10 pb-20 lg:pb-6 pt-6 space-y-6">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="hidden items-center gap-3 whitespace-nowrap">
          <Link href="/refundpolicyby3musafir" className="text-sm text-brand-primary hover:underline">
            Refund policy
          </Link>
          ...
          <span className="text-border">|</span>
          <Link href="/terms&conditonsby3musafir" className="text-sm text-brand-primary hover:underline">
            Terms &amp; Conditions
          </Link>
        </div>
      ...
      {showPolicies && (
        <div id="wallet-policy-links" className="-mt-3 rounded-xl border border-border bg-card p-3">
      ...
      <section className="rounded-xl border border-border bg-card p-4">
        ...
        <div className="mt-1 text-3xl font-bold text-heading">
          {walletLoading ? "—" : `Rs.${Number(walletSummary?.balance || 0).toLocaleString()}`}
        </div>
      ...
      <section className="rounded-xl border border-border bg-card p-4 space-y-3">
        <div className="flex items-center justify-between">
          ...
          {walletSummary?.whatsappTopupNumber && (
            <div className="text-xs text-text-light">WhatsApp: {walletSummary.whatsappTopupNumber}</div>
          )}
        </div>
